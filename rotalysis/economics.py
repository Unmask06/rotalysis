...
class Economics:
    ...
    def create_cashflow_df(capex, project_life):
        ...
        dfcashflow = pd.DataFrame(data={"fuel_cost": 0, "opex": 0, "cashflow": 0}, index=range(0, project_life))
        dfcashflow = dfcashflow.rename_axis("year")
        dfcashflow["cashflow"][0] = -capex
        return dfcashflow

    @staticmethod
    def inflation_adjusted_opex(opex, inflation_rate, years):
        adjusted_opex = [opex * ((1 + inflation_rate) ** (year - min(years))) for year in years]
        return adjusted_opex

    @staticmethod
    def annualized_spending(NPV, discount_rate, project_life):
        annualized_spending = -NPV * (discount_rate / (1 - (1 + discount_rate) ** (-project_life)))
    ...
    def calculate_payback_period(cash_flows):
        ...
        return int(payback_period) if isinstance(payback_period, (int,float)) else "Never"
